          ...
          {announcements?.map((a) => (
            <Card key={a.id} className="!p-6">
              <div className="flex flex-wrap items-center gap-3">
                <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${categoryStyles[a.category] ?? categoryStyles.General}`}>
                  {a.category}
                </span>
              ...
              <h2 className="mt-3 font-serif text-xl font-semibold text-navy-950">{a.title}</h2>
              <p className="mt-2 text-sm leading-relaxed text-navy-700/70">{a.summary}</p>
            </Card>
          ))}
        </div>
      </Section>
