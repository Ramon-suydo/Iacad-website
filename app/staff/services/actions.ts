...
export async function saveService(_prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  const { error } = await queueOrApplyChange({ supabase, userId: user.id, role,
    table: "services", operation: id ? "update" : "insert", recordId: id, payload, title: name });
  if (error) return { error: friendlyDbError(error, "A service") };

  revalidatePath("/staff/services");
  revalidatePath("/services");
  revalidatePath("/");
  redirect(`/staff/services?notice=${saveNotice(role)}`);
}
...
export async function deleteService(formData: FormData) {
  ...
  const { error } = await queueOrApplyChange({ supabase, userId: user.id, role,
    table: "services", operation: "delete", recordId: id, payload: {}, title: "Service" });
